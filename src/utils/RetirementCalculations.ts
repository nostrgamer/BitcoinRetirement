...
/**
 * Test if a portfolio can survive a conservative bear-market sequence:
 * 2 years at the Power Law floor, then 1 recovery year, then 20 years of runway.
 * Strategy: Use cash first during drawdowns to preserve Bitcoin.
 */
export const testBearMarketSurvival = (
  ...
  const fundWithdrawal = (bitcoinPriceForYear: number): boolean => {
    ...
    remainingCash = 0;
    remainingBitcoin -= remainingNeeded / bitcoinPriceForYear;
  ...
  // Years 1 and 2: Deep bear market at Power Law floor.
  if (!fundWithdrawal(deepBearPrice)) {
    return { passes: false, remainingBitcoin: 0, remainingCash: 0 };
  }
  if (!fundWithdrawal(deepBearPrice)) {
    return { passes: false, remainingBitcoin: 0, remainingCash: 0 };
  ...
  // Year 3: Bear market recovery - price between floor and fair value.
  const bearRecoveryPrice = floorValue + (fairValue - floorValue) * 0.75;
  if (!fundWithdrawal(bearRecoveryPrice)) {
    return { passes: false, remainingBitcoin: 0, remainingCash: 0 };
  }

  // Year 4+: Back to fair value - check runway.
  const remainingBitcoinValue = remainingBitcoin * fairValue;
  const totalRemainingValue = remainingBitcoinValue + remainingCash;
  const yearsOfRunway = totalRemainingValue / annualWithdrawal;
  const minimumYearsRequired = 20; // Must survive bear market plus 20 years of runway.
...
export const calculateMonthlySavingsProjection = (
  ...
  for (let year = 0; year < yearsToRetirement; year++) {
    for (let month = 0; month < 12; month++) {
      ...
      // Bear market in years 0 and 1 of each 4-year cycle, but not in the current calendar year.
      const isBearMarketYear = actualYear > startDate.getFullYear() && (cycleYear === 0 || cycleYear === 1);
      ...
      if (actualYear === startDate.getFullYear()) {
        // For current calendar year, use fair value (same as component logic)
